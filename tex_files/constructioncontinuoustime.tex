...
\begin{solution}
 ...
 When $t=A_{A(t)}$, i.e., when $t$ is the arrival time of the $A(t)$th job, we set $V(t) = V(A_{A(t)}) = \W_{A(t)}$ such that the virtual waiting time at the arrival time $t=A_{A(t)}$ is equal to the waiting time of the $A(t)$th job.
...
\begin{exercise}\label{ex:55}
  With the recursions~\cref{eq:59} it is apparently easy to compute the waiting time (in queue), but it is more difficult to compute the number of jobs in queue or in the system.
...
\begin{solution}
 ...
 \begin{equation*}
 \Q(A_k-) = \Q(A_{k-1}-) + 1 - \sum_{i= k-1 - \Q(A_{k-1}-)}^{k-1} \1{\tilde A_i < A_k}.
 \end{equation*}
